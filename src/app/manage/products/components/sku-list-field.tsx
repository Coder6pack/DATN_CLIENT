...
//       }
//     }
//   }, [combinations, onChange, variants,virtualPrice]); // Không phụ thuộc vào value
...
export default function SkuListField({
  ...
  const skus = useMemo((): SkuItem[] => {
    ...
    return combinations.map((combo) => {
      ...
      return {
        value: combo,
        price: virtualPrice, // Hardcode price to virtualPrice
        stock: existingSku?.stock ?? 100, // Default stock to 100 if not set
        image: existingSku?.image,
      };
    });
  ...
  const updateSkuStock = useCallback(
    (index: number, stock: string) => {
      const numericStock = stock ? Number.parseInt(stock, 10) : 100; // Default to 100 if empty
      const updatedSkus = skus.map((sku, i) =>
        i === index ? { ...sku, stock: numericStock } : sku
      );
      onChange(updatedSkus);
          ...
          >
            ...
            <div>
              <Input
                type="text"
                className="w-full"
                value={virtualPrice.toLocaleString()} // Display virtualPrice
            ...
            <div>
              <Input
                ...
                placeholder="100"
                className="w-full"
                value={sku.stock ?? 100} // Default to 100
                onChange={(e) => updateSkuStock(index, e.target.value)}
              />
            </div>
            <div>
