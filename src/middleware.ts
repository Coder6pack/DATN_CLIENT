...
export function middleware(request: NextRequest) {
  ...
  if (refreshToken) {
    ...
    // Restrict /manage/accounts to Admin role only
    if (pathname.startsWith("/manage/accounts") && role !== Role.Admin) {
      console.log(
        `Middleware: Redirecting to / due to insufficient role (${role}) for ${pathname}`
      );
